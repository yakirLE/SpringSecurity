...
public class PasswordRepositoryImpl implements PasswordRepository {
    ...
    public void saveToken(ResetToken resetToken) {
        ...
        template.update("INSERT INTO reset_tokens (email, username, token, expiry_date) VALUES " +
                ...
                resetToken.getToken(),
                resetToken.calculateExpiryDate(TokenAbstract.EXPIRATION));
    }

    @Override
    public ResetToken findByToken(String token) {
        JdbcTemplate template = new JdbcTemplate(dataSource);
        ResetToken resetToken = template.queryForObject("select email, username, token, expiry_date from reset_tokens where token = ?",
